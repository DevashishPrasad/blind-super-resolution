...
def main():
    ...
    # Prepare model
    model = BlindSR().cuda()
    model = torch.load("../../udrl_x4_172_1.67067.pth", map_location=torch.device("cuda:0"))
    model.eval()
    ...
    for dataset in datasets:
        ...
        for lr_path, hr_path in tqdm(zip(lr_paths, hr_paths),total=len(lr_paths)):
            ...
            if idx % 50 == 0:
                ...
                cv2.imwrite(f'../../udrl_results/{idx}_sr.png', sr)
                cv2.imwrite(f'../../udrl_results/{idx}_hr.png', hr)
                cv2.imwrite(f'../../udrl_results/{idx}_lr.png', save_lr)
