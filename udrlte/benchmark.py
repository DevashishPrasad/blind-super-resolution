...
def main():
    ...
    for dataset in datasets:
        ...
        for lr_path, hr_path in tqdm(zip(lr_paths, hr_paths),total=len(lr_paths)):
            ...
            # inference
            # sr = model(lr)
            # patch = torch.zeros([1, 3, 48, 48]).cuda()
            # for i in range(10):
            #     patch += get_patch(lr)
            # patch /= 10
            ...
            if idx % 50 == 0:
                ...
                cv2.imwrite(f'../../udrlte_results/{idx}_sr.png', sr)
                cv2.imwrite(f'../../udrlte_results/{idx}_hr.png', hr)
                cv2.imwrite(f'../../udrlte_results/{idx}_lr.png', save_lr)
